// src/app/(app)/pain/sections/Section3PainHistory.tsx
"use client";
...
export default function Section3PainHistory({ clientId, areaKey }: Props) {
  ...
  const points = useMemo(() => {
    ...
    // bygg serie for alle dager i range (og hull blir null)
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${w} ${h}`} className="w-full min-w-[700px]">
        ...
        {/* line */}
        {d ? (
          <path d={d} fill="none" stroke="#007C80" strokeWidth="3" />
        ) : null}
        ...
        {xs.map((x, i) => {
          const y = ys[i];
          if (y == null) return null;
          return <circle key={i} cx={x} cy={y} r="4.5" fill="#ffffff" stroke="#007C80" strokeWidth="2" />;
        })}
      </svg>
    </div>
  );
